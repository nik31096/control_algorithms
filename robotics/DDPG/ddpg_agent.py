import torch

from collections import OrderedDict
from copy import deepcopy

from .utils import PolicyNetwork, ValueNetwork


class DDPG:
    def __init__(self,
                 observation_space_shape,
                 goal_space_shape,
                 action_space_shape,
                 action_ranges,
                 gamma,
                 tau,
                 mode='multi_env'):
        self.gamma = gamma
        self.tau = tau
        self.mode = mode

        args = (observation_space_shape, goal_space_shape, action_space_shape, action_ranges)
        # online networks
        self.actor_gpu = PolicyNetwork(*args).to('cuda')
        self.actor_cpu = PolicyNetwork(*args)
        # syncronize gpu and cpu versions of online policy network
        self.actor_cpu.load_state_dict(self.actor_gpu.state_dict())
        self.actor_gpu.to('cuda')
        self.critic = ValueNetwork(*args[:-1]).to('cuda')

        # target networks
        self.target_actor = PolicyNetwork(*args).to('cuda')
        self.target_critic = ValueNetwork(*args[:-1]).to('cuda')

        # policy networks update
        self._hard_update(self.target_actor, self.actor_gpu)
        # value networks update
        self._hard_update(self.target_critic, self.critic)

        self.policy_opt = torch.optim.Adam(self.actor_gpu.parameters(), lr=1e-4)
        self.value_opt = torch.optim.Adam(self.critic.parameters(), lr=1e-3, weight_decay=1e-2)

    def select_action(self, states, desired_goals, achieved_goals):
        actions = self.actor_cpu(states, desired_goals, achieved_goals)

        return actions

    def train(self, batch):
        obs, des_goals, ach_goals, actions, rewards, next_obs, next_des_goals, next_ach_goals, dones = batch
        q_values = self.critic(obs, des_goals, ach_goals, actions)
        q_n_values = self.target_critic(next_obs, next_des_goals, next_ach_goals,
                                        self.target_actor(next_obs, next_des_goals, next_ach_goals).detach())

        y = rewards + self.gamma * (1 - dones) * q_n_values

        self.value_opt.zero_grad()
        value_loss = torch.mean((q_values - y) ** 2)
        value_loss.backward()
        self.value_opt.step()

        policy_loss = -torch.mean(self.critic(obs, des_goals, ach_goals, self.actor_gpu(obs, des_goals, ach_goals)))

        policy_loss.backward()
        self.policy_opt.step()
        self.policy_opt.zero_grad()

        # policy networks update
        self._soft_update(self.target_actor, self.actor_gpu, self.tau)
        # value networks update
        self._soft_update(self.target_critic, self.critic, self.tau)

        return value_loss.cpu().data.numpy(), -policy_loss.cpu().data.numpy()

    def syncronize_online_networks(self):
        state_dict = deepcopy(self.actor_gpu.state_dict())
        self.actor_cpu.load_state_dict(OrderedDict({key: state_dict[key].cpu() for key in state_dict}))

    def _hard_update(self, target, online):
        for target_param, param in zip(target.parameters(), online.parameters()):
            target_param.data.copy_(param.data)

    def _soft_update(self, target, online, tau):
        for target_param, param in zip(target.parameters(), online.parameters()):
            target_param.data.copy_((1 - tau)*target_param + tau*param)

    def save_models(self, path, model_name):
        torch.save(self.actor_gpu.state_dict(), f'{path}/policy_{model_name}.pt')
        torch.save(self.critic.state_dict(), f'{path}/value_{model_name}.pt')

    def load_pretrained(self, path, model_name):
        self.actor_gpu.load_state_dict(torch.load(f'{path}/policy_{model_name}.pt'))
        self.actor_cpu.load_state_dict(
            torch.load(f'{path}/policy_{model_name}.pt', map_location=torch.device('cpu')))
        self.critic.load_state_dict(torch.load(f'{path}/value_{model_name}.pt'))
        # policy networks update
        self._hard_update(self.target_actor, self.actor_gpu)
        # value networks update
        self._hard_update(self.target_critic, self.critic)
