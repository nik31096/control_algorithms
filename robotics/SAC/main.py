...
def main(mode, device):
    ...
    if mode == 'multi_env':
        ...
        replay_buffer = HindsightExperienceReplay(env_params=env_params, size=1000000, n_envs=n_envs,
                                                  use_achieved_goal=False, k=16)

        agent = SACAgent(observation_dim=envs.observation_space["observation"].shape[0],
        ...
        for epoch in trange(n_epochs):
            for step in range(1000):
                ...
                if epoch > epoch_delay:
                    ...
                    writer.add_scalar("Entropy_loss", entropy_loss, n_envs*(epoch*n_steps + step))
        ...
        agent = SACAgent(observation_dim=env.observation_space["observation"].shape[0],
